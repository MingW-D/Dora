      ...
      for (const fileName of Object.keys(zip.files)) {
        ...
        if (fileName.match(/ppt\/slides\/slide[0-9]+\.xml/)) {
          ...
          if (slideNumberMatch?.[1]) {
            ...
            const result = (await parser.parseStringPromise(xmlContent)) as XmlContent;
        ...
        if (fileName.match(/ppt\/media\/.+/)) {
          // 提取图片文件名和类型
          const imgName = this.getBaseName(fileName);
          const fileExt = this.getExtName(fileName).toLowerCase();
    ...
    try {
      ...
      for (const rel of relationshipsArray) {
        ...
        if (target && type && type.includes('image')) {
          // 从路径中提取图片名称
          const imgName = this.getBaseName(target);
