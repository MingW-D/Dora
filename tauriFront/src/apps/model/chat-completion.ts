...
export class ChatCompletion {
  ...
  private incrementalContentSubject: ReplaySubject<string> = new ReplaySubject<string>();
  ...
  private streamUsage: OpenAI.Chat.Completions.CompletionUsage = {
    prompt_tokens: 0,
    completion_tokens: 0,
    total_tokens: 0,
  };
  ...
  constructor(
    ...
    if (this.isStream) {
      ...
      this.incrementalContentSubject.next(this.content);
      ...
      for await (const chunk of this.streamResponse) {
        ...
        if (chunk.choices[0]?.delta?.tool_calls) {
          if (!hasToolCalls && !hasContent) {
            ...
            this.incrementalContentSubject.next('');
            this.contentSubject.complete();
            this.incrementalContentSubject.complete();
          }
        ...
        if (content) {
          ...
          this.contentSubject.next(this.content);  // Full accumulated content
          this.incrementalContentSubject.next(content);  // Only the new chunk
        }
        ...
        // 收集token使用情况（在流的最后一个chunk中）
        if (chunk.usage) {
          this.streamUsage = chunk.usage;
        }
      }
  ...
  // 获取增量内容流 (仅返回新增的内容块)
  get incrementalContentStream() {
    return this.incrementalContentSubject.asObservable();
  }
